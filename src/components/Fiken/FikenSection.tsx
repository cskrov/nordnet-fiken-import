...
import { isInnskuddLine } from '@app/lib/fiken/guards';
...
const WithFirstLine: VoidComponent<FikenSectionWithFirstLineProps> = ({
  ...
  const generatePreviousMonth = () => {
    ...
    const [konto, setKonto] = createSignal<string | null>(
      isInnskuddLine(_firstLine) ? _firstLine.tilKonto() : _firstLine.fraKonto(),
    );
