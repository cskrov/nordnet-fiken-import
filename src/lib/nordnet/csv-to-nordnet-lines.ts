...
import { NordnetErrorType, NordnetType, isNordnetType, type NordnetError, type NordnetLine, type RawNordnetLine } from "$lib/nordnet/types";
...
export const NORDNET_HEADER_FORKLARENDE_TEKST = "Transaksjonstekst";
...
export const toNordnetLines = (csvFiles: CsvFile[]): NordnetLine[] => {
  ...
  const rawNordnetRows = csvFiles.flatMap(({ fileName, data }) => {
    ...
    const idIndex = headers.indexOf(NORDNET_HEADER_ID);
    const bokførtDatoIndex = headers.indexOf(NORDNET_HEADER_BOKFORT_DATO);
    const kontoIndex = headers.indexOf(NORDNET_HEADER_KONTO);
    const beløpIndex = headers.indexOf(NORDNET_HEADER_BELØP);
    const saldoIndex = headers.indexOf(NORDNET_HEADER_SALDO);
    const forklarendeTekstIndex = headers.indexOf(NORDNET_HEADER_FORKLARENDE_TEKST);
    const typeIndex = headers.indexOf(NORDNET_HEADER_TYPE);
    ...
    return rows.map<RawNordnetLine>((row, rowNumber) => {
      ...
      return ({
        ...
        forklarendeTekst: row[forklarendeTekstIndex],
  ...
  const nordnetLines = rawNordnetRows.map<NordnetLine>((row) => {
    const { id, bokførtDato, portefølje, transaksjonstype, beløp, saldo, forklarendeTekst, source } = row;
